...
export type ListDmsFoldersQuery = (
  ...
  & { listDmsFolders?: Maybe<Array<(
    { __typename?: 'DmsFolder' }
    & Pick<DmsFolder, 'id' | 'entityId' | 'companyId' | 'foldername' | 'entityType' | 'type' | 'order' | 'deletedAt'>
  )>> }
);
...
export const ListQuestionaireGroupsDocument = gql`
  ...
  listQuestionaireGroups(templateId: $templateId) @rest(type: "ListQuestionaireGroups", path: "/groups?templateId={args.templateId}", method: "GET", endpoint: "default") {
    id
    templateId
    groupName
    copyFromId
