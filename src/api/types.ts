...
export type GetCompanyDetailsQuery = (
  ...
  & { getCompanyDetails: (
    { __typename?: 'Company' }
    & Pick<Company, 'id' | 'name' | 'domain' | 'address' | 'state' | 'city' | 'zipcode' | 'country' | 'houseNumber' | 'floor' | 'chamberOfCommerce' | 'vat'>
    & { image?: Maybe<(
      { __typename?: 'File' }
      & Pick<File, 'url' | 'id'>
    )> }
...
export type ListEmailFoldersQuery = (
  ...
  & { listEmailFolders?: Maybe<Array<(
    { __typename?: 'EmailFolderListItem' }
    & Pick<EmailFolderListItem, 'numberOfEmails'>
    & { folder: (
      { __typename?: 'EmailFolder' }
      & Pick<EmailFolder, 'name' | 'displayName'>
    ) }
  )>> }
);
