...
import { Avatar, Box, Card, CardContent, CardHeader, Emoji, ProgressFilling, Typography } from 'ui/atoms';
...
export const QuotationItem = ({
  item: { address, avatar, documents, informationCompletedStatus },
}: {
  item: Quotation;
}) => {
  ...
  return (
    <Box display="flex" width="100%" className={classes.quotationItem}>
      <Box mr={2}>
        <Avatar variant="rounded" size="medium" src={avatar} className={classes.quotationImage}>
          {!avatar && <Emoji>{'📷'}</Emoji>}
        </Avatar>
      </Box>
      <Box width="100%">
        <Typography variant="h5" className={classes.quotationTitle}>
          {address}
        </Typography>
        <Box display="flex" mt={2}>
          <Box mr={3.5}>
            <Typography variant="h6" className={classes.quotationLabel}>
              {formatMessage({
                id: 'crm.details.sales.dashboard.quotations.info_completed',
              })}
            </Typography>
          ...
          <Box>
            <Typography variant="h6" className={classes.quotationLabel}>
              {formatMessage({
                id: 'crm.details.sales.dashboard.quotations.documents',
              })}
            </Typography>
...
export const QuotationsBoard = () => {
  ...
  return (
    <Card>
      <CardHeader title={formatMessage({ id: 'crm.details.sales.dashboard.quotations' })} />
      <CardContent>
        {quotations.map((quotation, index) => (
          <QuotationItem key={index} item={quotation} />
        ))}
      </CardContent>
    </Card>
  );
