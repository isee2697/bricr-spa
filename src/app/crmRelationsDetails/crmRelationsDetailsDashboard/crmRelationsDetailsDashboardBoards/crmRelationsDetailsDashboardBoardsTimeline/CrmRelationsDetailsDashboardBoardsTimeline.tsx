...
export const CrmRelationsDetailsDashboardBoardsTimeline = () => {
  ...
  return (
    <Card>
      ...
      />
      <CardContent>
        <Stepper orientation="vertical" connector={<TimelineStepConnector className={classes.stepConnector} />}>
              ...
              >
                <Box display="flex" className={classes.stepLabelContent}>
                  <Box mr={3}>
                    <Typography
                      variant="h5"
                      className={clsx(classes.timelineStatus, timeline.status === 'done' ? classes.green : classes.red)}
                    >
                      {timeline.status === 'done'
                        ? formatMessage({
