  ...
  return (
    <>
      <Card className={classes.root}>
        <CardHeader
          title={formatMessage({ id: 'pim_details.allocate_results.filtering_properties.title' })}
          action={
            <FormControlLabel
            ...
            <Grid item xs={12}>
              <Box>
                <Typography variant="h3">
                  {formatMessage({
                    id: 'pim_details.allocate_results.filtering_properties.price_range',
                  })}
                </Typography>
                <Grid container spacing={1}>
                  <Grid item xs={4}>
                    <GenericField
                      name="rentPriceFrom"
                      label={formatMessage({
                        id: 'pim_details.allocate_results.filtering_properties.rent_price_from',
                      })}
                      placeholder={formatMessage({
                        id: 'pim_details.allocate_results.filtering_properties.rent_price_from.placeholder',
                      })}
                  ...
                  <Grid item xs={4}>
                    <GenericField
                      name="rentPriceTill"
                      label={formatMessage({
                        id: 'pim_details.allocate_results.filtering_properties.rent_price_till',
                      })}
                      placeholder={formatMessage({
                        id: 'pim_details.allocate_results.filtering_properties.rent_price_till.placeholder',
                      })}
                      InputProps={{ endAdornment: <EuroIcon /> }}
                      disabled={!isEditing}
                    />
                ...
                <Grid container spacing={1}>
                  <Grid item xs={4}>
                    <GenericField
                      name="salesPriceFrom"
                      label={formatMessage({
                        id: 'pim_details.allocate_results.filtering_properties.sales_price_from',
                      })}
                      placeholder={formatMessage({
                        id: 'pim_details.allocate_results.filtering_properties.sales_price_from.placeholder',
                      })}
                      InputProps={{ endAdornment: <EuroIcon /> }}
                      disabled={!isEditing}
                    />
              ...
              <Box className={classes.marginTopFour}>
                <Typography variant="h3">
                  {formatMessage({
                    id: 'pim_details.allocate_results.filtering_properties.published',
                  })}
                </Typography>
                <CheckboxField
                  name="onlyPublishedProperties"
                  label={formatMessage({
                    id: 'pim_details.allocate_results.filtering_properties.only_published_properties',
                  })}
                />
              </Box>
              <Box className={classes.marginTopFour}>
                <Box display="flex" justifyContent="space-between">
                  <Typography variant="h3">
                    {formatMessage({
                      id: 'pim_details.allocate_results.filtering_properties.project',
                    })}
                  </Typography>
                  <Typography variant="h5" className={classes.gray}>
                    {formatMessage({
                      id: 'pim_details.allocate_results.filtering_properties.search_one_or_more_projects_below',
                    })}
                  </Typography>
                </Box>
                <TextField
                  ...
                  {projects.map(project => (
                    <Box display="flex" alignItems="center" className={classes.project}>
                      <Typography variant="h5" className={classes.projectLabel}>
                        {formatMessage({ id: 'pim_details.allocate_results.filtering_properties.project' })}:
                      </Typography>
              ...
              <Box className={classes.marginTopFour}>
                <Box display="flex" justifyContent="space-between">
                  <Typography variant="h3">
                    {formatMessage({
                      id: 'pim_details.allocate_results.filtering_properties.properties',
                    })}
                  </Typography>
                  <Typography variant="h5" className={classes.gray}>
                    {formatMessage({
                      id: 'pim_details.allocate_results.filtering_properties.search_one_or_more_properties_below',
                    })}
                  </Typography>
                </Box>
                <TextField
                  ...
                  placeholder={formatMessage({
                    id: 'pim_details.allocate_results.filtering_properties.search_properties_of_this_projects',
                  })}
                  className={classes.searchField}
                  onChange={handleChangeProjectKey}
                />
                <Box display="flex" flexWrap="wrap" className={classes.projectsWrapper}>
                  {properties.map(project => (
                    <Box display="flex" alignItems="center" className={classes.project}>
                      <Typography variant="h5" className={classes.projectLabel}>
                        {formatMessage({ id: 'pim_details.allocate_results.filtering_properties.property' })}:
                      </Typography>
                      <Typography variant="h5" className={classes.projectTitle}>
                        {project.title}
                      </Typography>
      ...
      <Box display="flex" justifyContent="space-between" width="100%" onClick={onPreviousStep}>
        <Button variant="outlined" color="primary">
          {formatMessage({ id: 'pim_details.allocate_results.filtering_properties.go_to_settings' })}
        </Button>
        <Button variant="outlined" color="primary" onClick={onNextStep}>
          {formatMessage({ id: 'pim_details.allocate_results.filtering_properties.go_to_filtering_people' })}
        </Button>
      </Box>
    </>
  );
