...
import { AddQuestionnaireItemModalProps } from './AddQuestionnaireItemModal.types';
import { questionTypes } from './dictionaries';

export const AddQuestionnaireItemModal = ({ onSubmit }: AddQuestionnaireItemModalProps) => {
  const { formatMessage } = useLocale();
  const [multipleChoiceAnswers, setMultipleChoiceAnswers] = useState<number[]>([]);
  const { close } = useModalDispatch();
  const { isOpen } = useModalState('add-questionnaire-item');

  return (
    <FormModal
      isOpened={isOpen}
      onClose={() => close('add-questionnaire-item')}
      onSubmit={onSubmit}
