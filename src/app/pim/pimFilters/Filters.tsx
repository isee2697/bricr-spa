...
import { BuildingIcon, NewConstructionIcon, SquareIcon } from 'ui/atoms/icons';
import { Box, Grid, Alert, DialogContent, DialogActions, UserAvatar } from 'ui/atoms';
...
const defaultFilters: FiltersTypes[] = [
  {
    key: 'city',
    type: Types.Text,
    size: Sizes.L,
  ...
  useEffect(() => {
    setFilters([
      ...
      },
      {
        ...
        type: Types.Checkbox,
        size: Sizes.M,
        options: accountManagers.map(({ id, firstName, lastName, image }) => ({
          label: `${firstName} ${lastName}`,
          value: id,
          icon: <UserAvatar avatar={image?.url || ''} name={`${firstName} ${lastName}`} />,
        })),
      },
    ]);
  }, [accountManagers, teams]);
