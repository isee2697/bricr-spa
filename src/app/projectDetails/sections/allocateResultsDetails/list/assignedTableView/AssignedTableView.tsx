...
import { NcpAllocateAssignedTableFilters } from './dictionaries';
  ...
  const headerCells: AssignedTableHeaderCell[] = [
    {
      ...
      renderer: property => `${property.partner.firstName} ${property.partner.lastName}`,
    },
    {
      field: 'dateOfSubscription',
      label: formatMessage({ id: 'project_details.allocateResults.assigned.date' }),
      renderer: property => (
        <Typography variant="h5">{property.dateOfSubscription?.toFormat('dd-MM-yyyy') || ''}</Typography>
      ),
    },
    {
    ...
    {
      field: 'maximumRentalPrice',
      label: formatMessage({ id: 'project_details.allocateResults.assigned.price' }),
      renderer: property => `€ ${property.maximumRentalPrice}`,
    },
  ];
        ...
        title={
          <Box ml={8} display="flex" alignItems="center">
            ...
            <Typography variant="h2">
              {formatMessage({ id: `pim_details.allocate_results_details.ranking_relations` })}
            </Typography>
          </Box>
        }
        action={
          ...
          <ListHeader
            ...
            actionButtons={
              <FiltersButton
                ...
                filters={NcpAllocateAssignedTableFilters}
              />
            }
          />
          ...
          <Table>
            <TableBody>
              {properties.map((item, index) => (
                <TableRow
                  key={index}
                  onClick={() => onClick?.(String(item.ranking))}
                  className={classnames(classes.tableRow)}
                >
                  <TableCell padding="checkbox">
                    <Checkbox
                      color="primary"
                      checked={selected.includes(String(item.ranking))}
                      inputProps={{ 'aria-labelledby': String(item.ranking) }}
                      onClick={e => {
                        e.stopPropagation();
                        onSelectProperty(String(item.ranking));
                      }}
                    />
                  </TableCell>
                  ...
                  <TableCell className={classes.tableCellActions}>
                    <ListOptionsMenu
                      id={`allocate-results-details-${type}-${item.ranking}`}
                      onDeleteClick={() => {}}
                      hideEditButton
                    >
                      <ListOptionsMenuItem
