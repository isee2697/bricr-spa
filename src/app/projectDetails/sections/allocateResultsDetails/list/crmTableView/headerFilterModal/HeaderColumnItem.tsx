...
export const HeaderColumnItem = ({ item, isShow, setShow, isDisabled, changeOrder }: HeaderColumnItemProps) => {
  ...
  const [{ isDragging }, drag] = useDrag({
    item: {
      type: 'AllocateResultsTableHeaderFilter',
      item,
    },
    collect: monitor => ({
  ...
  const [{ isOver, isDrag }, drop] = useDrop({
    accept: 'AllocateResultsTableHeaderFilter',
        ...
        >
          ...
          <Box ml={2}>
            <Typography variant="h5" color={isDisabled ? 'textSecondary' : 'primary'}>
              {formatMessage({ id: `project_details.allocate_results.header_filter.column.${item}` })}
            </Typography>
          </Box>
        </Box>
