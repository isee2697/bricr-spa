...
export const Tags = () => {
  ...
  return (
    <Card>
      <CardHeader
        title={formatMessage({ id: 'project_details.allocate_results.tags.title' })}
        action={
          <>
            <FormControlLabel
        ...
        <Box>
          <Box display="flex" justifyContent="space-between" mb={1}>
            <Typography variant="h5">
              {formatMessage(
                { id: 'project_details.allocate_results.tags.relation_have_tag' },
                { strong: <b>{formatMessage({ id: 'project_details.allocate_results.tags.must' })}</b> },
              )}
            </Typography>
            <Typography variant="h5" className={classes.gray}>
              {formatMessage({ id: 'project_details.allocate_results.tags.search_one_or_more_tags_below' })}
            </Typography>
          </Box>
        ...
        <Box mt={4}>
          <Box display="flex" justifyContent="space-between" mb={1}>
            <Typography variant="h5">
              {formatMessage(
                { id: 'project_details.allocate_results.tags.relation_have_tag' },
                { strong: <b>{formatMessage({ id: 'project_details.allocate_results.tags.must_not' })}</b> },
              )}
            </Typography>
            <Typography variant="h5" className={classes.gray}>
              {formatMessage({ id: 'project_details.allocate_results.tags.search_one_or_more_tags_below' })}
