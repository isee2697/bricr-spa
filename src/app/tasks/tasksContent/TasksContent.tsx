  ...
  return (
    <Grid container className={classes.root} direction="column">
      ...
      >
        <Grid item xs={3}>
          ...
          {tasksCount > completedTasksCount && (
            <TasksStatusMessage tasks={tasksCount} done={completedTasksCount} />
          )}
        </Grid>
        <Grid item>
          <Grid container>
            ...
            <IconButton
              ...
              onClick={() => setViewMode(TasksViewMode.Swimlane)}
            >
              <SwimlaneIcon
                color={
                  viewMode === TasksViewMode.Swimlane ? "primary" : "inherit"
                }
              />
            </IconButton>
            <IconButton
              classes={{ root: classes.sortIcon }}
              onClick={() => setViewMode(TasksViewMode.List)}
            >
              <ListIcon
                color={viewMode === TasksViewMode.List ? "primary" : "inherit"}
              />
            </IconButton>
      ...
      <Grid item xs={12}>
        <TasksDateSection
          tab={tab}
          handleSetDateRange={(range: DateRange) => setDateRange(range)}
        />
      </Grid>
      <Grid
        item
