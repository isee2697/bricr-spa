...
export const DmsDashboardBoardsAgenda = () => {
  ...
  const sortedByDate = Object.keys(grouped).sort((dateA, dateB) => {
    return DateTime.fromString(dateA, dateFormat) >
      DateTime.fromString(dateB, dateFormat)
      ? 1
      : -1;
  });

  const getGroupAgendaItems = (items: AgendaItemProps[]) => {
    return items.map((itemProps, key) => (
      <AgendaItem key={key} {...itemProps} />
    ));
  };
  ...
  return (
    <Card className={classes.root}>
      ...
      >
        <Tab
          label={
            <div className={classes.agendaTab}>
              {formatMessage({ id: 'dms.dashboard.agenda.accepted' })}
            </div>
          }
        />
        <Tab
          label={
            <Badge badgeContent={3} color='secondary'>
              <div className={classes.agendaTab}>
                {formatMessage({ id: 'dms.dashboard.agenda.rejected' })}
              </div>
            </Badge>
          }
        />
