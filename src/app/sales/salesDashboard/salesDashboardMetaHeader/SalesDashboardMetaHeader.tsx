...
import { SalesDashboardMetaHeaderProps, SalesDashboardMetaItem } from './SalesDashboardMetaHeader.types';
import { useStyles } from './SalesDashboardMetaHeader.styles';

const SalesDashboardMetaBox = ({ item: { label, count, percentage, status } }: SalesDashboardMetaHeaderProps) => {
        ...
        <Box display="flex" justifyContent="space-between" alignItems="flex-start">
          ...
          <Box display="flex" flexDirection="column" alignItems="flex-end">
            ...
            <Typography variant="h6" className={classes.metaDays}>
              {formatMessage({ id: 'sales.dashboard.meta.previous_last_7_days' })}
            </Typography>
          </Box>
        </Box>
      </CardContent>
...
export const SalesDashboardMetaHeader = () => {
  const theme = useTheme();
  const classes = useStyles();

  const metaAsArray: SalesDashboardMetaItem[] = [
    {
      label: 'leads',
      count: 45,
      percentage: 15,
    ...
    <ScrollableHorizontal width="100%" maxWidth="100%" height={theme.spacing(12)}>
      <Box className={classes.metaWrapper}>
        {metaAsArray.map((meta, index) => (
          <SalesDashboardMetaBox key={index} item={meta} />
        ))}
      </Box>
