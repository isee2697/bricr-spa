...
import { SelectTemplateModalProps, EmailTemplateItem } from './SelectTemplateModal.types';

export const SelectTemplateModal = ({ isOpened, onClose, onSubmit, templates }: SelectTemplateModalProps) => {
  const { formatMessage } = useLocale();

  const filterItem = (item: EmailTemplateItem, currentValue: string) =>
  ...
  return (
    <FormModal
      ...
      addIcon={<></>}
      addText={formatMessage({ id: 'email.set_template' })}
    >
      <Field name="crm">
        {({ input }) => (
          <SearchList<EmailTemplateItem>
            items={templates}
            selectedItemsIds={[]}
