    ...
    <>
        ...
        <TableHead>
          <TableRow>
            <TableCell className={classes.tableHeaderCell}>
              {formatMessage({ id: 'crm.details.business_journey.current_offer' })}
            </TableCell>
            <TableCell className={classes.tableHeaderCell} />
            <TableCell className={classes.tableHeaderCell}>
              {formatMessage({ id: 'crm.details.business_journey.counter_offer' })}
            </TableCell>
            <TableCell className={classes.tableHeaderCell}>
              {formatMessage({ id: 'crm.details.business_journey.conditions' })}
            </TableCell>
            <TableCell className={classes.tableHeaderCell}>
          ...
          {counters.map((counter, index) => (
            <TableRow key={index}>
              ...
              <TableCell className={clsx(index === counters.length - 1 && classes.tableCellNoBorderBottom)}>
                ...
                >
                  {formatMessage({ id: 'crm.details.business_journey.condition.technical_building_inspection' })}
                </Typography>
                <Typography
                  variant="h6"
              ...
              <TableCell className={clsx(index === counters.length - 1 && classes.tableCellNoBorderBottom)}>
                <Typography className={clsx(classes.tableCellStatus, counter.status === 'counter' && 'error')}>
                  {formatMessage({ id: `crm.details.business_journey.status.${counter.status}` })}
                </Typography>
              </TableCell>
            </TableRow>
          ))}
      ...
      <Box className={classes.actions}>
        <Button variant="outlined" color="secondary" className={classes.actionButtons}>
          {formatMessage({ id: 'crm.details.business_journey.actions.counter_or_reject_offer' })}
        </Button>
        <Button variant="contained" color="primary" className={classes.actionButtons}>
          {formatMessage({ id: 'crm.details.business_journey.actions.accept_offer' })}
        </Button>
      </Box>
    </>
  );
