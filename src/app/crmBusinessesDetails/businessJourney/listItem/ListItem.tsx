...
import { BusinessJourneyTab } from '../BusinessJourney.types';
      ...
      <Box className={classes.rowItem}>
        <Box display="flex">
          ...
          <Box display="flex" flexDirection="column" width="100%">
                  ...
                  <Box>
                    ...
                    <Box display="flex">
                      <Typography variant="h6" className={clsx(classes.property, classes.fontWeightMedium)}>
                        {formatMessage({ id: 'crm.details.business_journey.room_size' }, { size })}
                      </Typography>
                      <Typography variant="h6" className={clsx(classes.property, classes.fontWeightMedium)}>
                        {formatMessage({ id: 'crm.details.business_journey.rooms' }, { rooms })}
                      </Typography>
            ...
            {status === BusinessJourneyTab.Viewings && item.brokerages && item.brokerages.length > 0 && (
              <ListItemViewings brokerages={item.brokerages} />
            )}
            {status === BusinessJourneyTab.Biddings && item.counters && item.counters.length > 0 && (
              <ListItemBiddings counters={item.counters} />
            )}
            {status === BusinessJourneyTab.Candidate && item.candidates && item.candidates.length > 0 && (
              <ListItemCandidates candidates={item.candidates} />
            )}
            {status === BusinessJourneyTab.Optant && item.optants && item.optants.length > 0 && (
