...
export const useSelect = <T>(items: T[], itemIndex: keyof T, disabled?: boolean, initialValues?: string[]) => {
  const [checkedKeys, dispatch] = useReducer<Reducer<SelectState, SelectAction>>(
    (state, action) => {
      switch (action.type) {
        case 'TOGGLE':
          if (!action.key) {
